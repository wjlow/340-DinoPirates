    ...
    public InputGroupingPanel(ModelInterface m)
    {
     ...
     public void actionPerformed(ActionEvent e)
     {
        if(e.getSource() == group_button)
        {
            ...
            String groupArray[][] = new String [2][5];

            String splitGroups [] = group1Ids.split(",");

            for(int j=0; j<groupArray.length; j++)
            {
                for(int i=0; i<splitGroups.length; i++)
                {
                    groupArray[i][j] = splitGroups[j];
                    System.out.println(groupArray[i][j]);
                    System.out.println(splitGroups[j]);
                }
